/**
 * KUtils create by Sungbin at 2020.07.01.
 *
 * Copyright (C) SungBin
 * all rights reserved.
 */
...
const KUtils = {
    ...
    removeFolder(path) {
        new File(sdcard + '/' + path).delete();
    }, 
    makeFile(path) {
        new File(sdcard + '/' + path).createNewFile();
    }, 
    makeRandom(min, max) {
...
//아무것도 하기 싫음니다.. . . . . 
const database = JSON.parse(FileStream.read('/sdcard/Kakao-RPG/database.json'));
if (database==null) database = []; // database는 사용자들의 정보가 담긴 객체를 담은 **배열**

/**
 * MD5 Encryption Function
 * @see https://stackoverflow.com/a/41602636
...
const MD5 = function (e) {
    ...
    f = function (b) {
        let a, c = b.length;
        a = c + 8;
        for (let d = 16 * ((a - a % 64) / 64 + 1), e = Array(d - 1), f = 0, g = 0; g < c;) a = (g - g % 4) / 4, f = g % 4 * 8, e[a] |= b.charCodeAt(g) << f, g++;
        a = (g - g % 4) / 4;
        e[a] |= 128 << g % 4 * 8;
        e[d - 2] = c << 3;
        e[d - 1] = c >>> 29;
...
// 예
// 아무것도 하기가싫음네다
/* 이런식으로 제 3자가 쉽게 봇을 바꿀 수 있게 하면 좋을 려낭? */
/* . . . . . */
/* 일해랏! WORKER!! */
...
const response = (room, msg, sender, isGroupChat, replier, ImageDB, packageName, threadId) => {
    if (msg=='!가입') {
